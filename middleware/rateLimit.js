...
// Rate limiter configurations (production-safe defaults)
const createLimiter = buildLimiter({
  keyPrefix: 'rl:create',
  points: 50,   // allow 50 requests/min for link creation
  duration: 60,
});

const redirectLimiter = buildLimiter({
  keyPrefix: 'rl:redirect',
  points: 1000, // allow 1000 requests/min for redirects
  duration: 60,
});
...
function rateLimitMw(rl) {
  return async (req, res, next) => {
    // Skip if no Redis or not in production
    ...
    try {
      ...
      console.warn(`Rate limit hit by ${req.ip}, retry in ${retrySecs}s`);
